...
\begin{figure}[h!]
  ...
  \end{subfigure}
  \caption[Applying Wiener filters to the 35 ton data]{The effect of applying Wiener filters to the 35 ton data.}
...
Once the hits on a track have been corrected in time and space the hits on noisy wires and delta rays must be removed as the change in hit RMS from diffusion will be sub-dominant for these hits. Wires with a high level of correlated noise observe hits with a wider RMS as shown by !!!FIXME!!!Figure~\ref{fig:DomsHitModel}!!! where a baseline noise of 10 ADC counts was added to a simulated hit with a peak value of 50 ADC counts and RMS 10 ticks. Hits with delta rays need to be removed as the deposited energy will be larger and over a longer period of time than hits from the main track, this will make the RMS of the individual hit wider and also increase the width of the charge distribution for the track. In order to remove these hits only hits which satisfy the following cuts are used:
\begin{itemize}
...
\begin{itemize}
\item The $RMS$ of the hit - needed to measure the transverse diffusion.
\item The $charge$ of the hit, measured as the integral of Gaussian fit - needed to measure the longitudinal diffusion.
\item The $RMS/charge$ of the hit - to attempt to remove the angular dependence of the track.
\end{itemize}
